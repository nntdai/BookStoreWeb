<?php
    ...
    include_once("C:/xampp/htdocs/BookStoreWeb/Controller/Admin/chitietgiohang.php");
    ...
    if(isset($_GET['madon'])){
        ...
        $chitietgio=$chitietgio_Control->getById($idgiohang);
    }
    if(isset($_GET['chitiet'])){
        include_once("C:/xampp/htdocs/BookStoreWeb/View/Admin/Pages/DonHang/chitietgiohang.php");
        exit; // dừng các mã chạy phía dưới;
    }
    if(isset($_GET['status'])){
        if(isset($_GET['startday']) && isset($_GET['endday'])){
            $Search_list=$Donhang_Control->getStt_D($_GET['status'], $_GET['startday'], $_GET['endday']);
        }else if(!isset($_GET['startday']) && !isset($_GET['endday'])){
            $Search_list=$Donhang_Control->getStatus($_GET['status']);
        }
    }else{
        if(isset($_GET['startday']) && isset($_GET['endday'])){
            $Search_list=$Donhang_Control->getDay($_GET['startday'], $_GET['endday']);
        }
    }
?>
<!DOCTYPE html>
...
<head>
	<link rel="stylesheet" href="/BookStoreWeb/Public/Admin/Pages/Home/donhang.css">
    <script src="/BookStoreWeb/Public/Admin/Pages/Home/js/DonHang/DonHang.js"></script>
	<meta charset="UTF-8"/>
	<title>list</title>
</head>
<body>
        ...
        <form method="POST" id="update_order" action="" onsubmit="return false">
            <div id="khung_update">
                <div style="margin:10px; text-align: center; height:26px">
                    <img src="/BookStoreWeb/Image/close.png" height="25px" width="25px" style="float:right" onclick="close_update_order()">
                </div>
                    <h2 style="margin-bottom:30px">Chi tiết đơn hàng</h2>
                <div class="intro_update">
                    <b>Mã đơn hàng :</b>
                    <input type="text" id="madonText_update" value="<?php echo "$madh";?>" name="madonText_update" size="50" class="input" readonly>
                </div>
                <div class="intro_update">
                    <b>Tên khách hàng :</b>
                    <input type="text" id="NameText_update" value="<?php echo "$ten";?>" name="NameText_update" size="50" class="input" readonly>
                </div>
                <div class="intro_update">
                    <b>Số điện thoại :</b>
                ...
                <div>
                    ...
                    <a href="index.php?controller=order&chitiet=<?php echo isset($_GET['madon']) ? $_GET['madon'] : '' ;?>" style="float:right; font-size:20px; margin-top:15px; margin-right:10px; text-decoration: none">Chi tiết giỏ hàng</a>
                </div>
            </div>
        </form>
    </div>
    <div id="container" >
        <br>
        <h2 style="text-align:left">Danh sách đơn hàng</h2>
        <div style="text-align:left; height:auto; margin-bottom:-15px; margin-left:10px">
            <b>Thời gian : </b>
            <input type="datetime-local" id="StartDay" class="search" style="width: 250px">
            <b> - </b>
            <input type="datetime-local" id="EndDay" class="search" style="width: 250px">
            <b>Tình trạng : </b>
            <select id="cboSttSearch" name="cboStt" class="search" style="width: 150px;">
        ...
        <?php
            if(isset($_GET['status']) || isset($_GET['startday']) || isset($_GET['endday'])){
                foreach ($Search_list as $dh){       
                    echo "<th>".$dh['madon']."</th>";
